...
class GetPetTypesSuccessResponseResultPetTypesPropertyValue(BaseModel):
    """
    Specific pet types for each property
    """ # noqa: E501
    attributes: Optional[GetPetTypesSuccessResponseResultPetTypesPropertyValueAttributes] = Field(default=None, alias="@attributes")
    pet_type: Optional[List[GetPetTypesSuccessResponseResultPetTypesPropertyValuePetTypeInner]] = Field(default=None, description="List of pet types available for the property", alias="PetType")
    __properties: ClassVar[List[str]] = ["@attributes", "PetType"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in pet_type (list)
        _items = []
        if self.pet_type:
            for _item in self.pet_type:
                if _item:
                    _items.append(_item.to_dict())
            _dict['PetType'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "@attributes": GetPetTypesSuccessResponseResultPetTypesPropertyValueAttributes.from_dict(obj["@attributes"]) if obj.get("@attributes") is not None else None,
            "PetType": [GetPetTypesSuccessResponseResultPetTypesPropertyValuePetTypeInner.from_dict(_item) for _item in obj["PetType"]] if obj.get("PetType") is not None else None
        })
        return _obj
