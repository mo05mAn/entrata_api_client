...
class GetPoReceivingRecordsSuccessResponseResultPurchaseOrdersPurchaseOrderPoDetailsPoDetail(BaseModel):
    """
    GetPoReceivingRecordsSuccessResponseResultPurchaseOrdersPurchaseOrderPoDetailsPoDetail
    """ # noqa: E501
    id: StrictStr = Field(description="Unique identifier for the purchase order detail.")
    catalog_item_name: StrictStr = Field(description="Name of the catalog item.", alias="catalogItemName")
    qty_ordered: StrictStr = Field(description="Quantity of items ordered.", alias="qtyOrdered")
    unit_cost: StrictStr = Field(description="Cost per unit of the catalog item.", alias="unitCost")
    receiving_records: GetPoReceivingRecordsSuccessResponseResultPurchaseOrdersPurchaseOrderPoDetailsPoDetailReceivingRecords = Field(alias="receivingRecords")
    cancelled_records: GetPoReceivingRecordsSuccessResponseResultPurchaseOrdersPurchaseOrderPoDetailsPoDetailCancelledRecords = Field(alias="cancelledRecords")
    __properties: ClassVar[List[str]] = ["id", "catalogItemName", "qtyOrdered", "unitCost", "receivingRecords", "cancelledRecords"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of receiving_records
        if self.receiving_records:
            _dict['receivingRecords'] = self.receiving_records.to_dict()
        # override the default output from pydantic by calling `to_dict()` of cancelled_records
        if self.cancelled_records:
            _dict['cancelledRecords'] = self.cancelled_records.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "receivingRecords": GetPoReceivingRecordsSuccessResponseResultPurchaseOrdersPurchaseOrderPoDetailsPoDetailReceivingRecords.from_dict(obj["receivingRecords"]) if obj.get("receivingRecords") is not None else None,
            "cancelledRecords": GetPoReceivingRecordsSuccessResponseResultPurchaseOrdersPurchaseOrderPoDetailsPoDetailCancelledRecords.from_dict(obj["cancelledRecords"]) if obj.get("cancelledRecords") is not None else None
        })
        return _obj
