...
class GetCalendarAvailabilityR2MethodParams(BaseModel):
    """
    ...
    from_date: date = Field(description="This is a required field. This field accepts single value. fromDate indicates from which date we should start to include when ret urning available time periods off the property calendar. Only a 60 d ay range is allowed per request and that range includes the fromDate and toDate in it.", alias="fromDate")
    to_date: date = Field(description="This is a required field. This field accepts single value. toDate indicates up till which date we should include when returning a vailable time periods off the property calendar. Only a 60 day range is allowed per request and that range includes the fromDate and toDate in it.", alias="toDate")
    tour_type: Optional[StrictStr] = Field(default=None, description="This is a required field. This field accepts single value. tour type: SELF_GUIDED, VIRTUAL_GUIDED,AGENT_GUIDED", alias="tourType")
    __properties: ClassVar[List[str]] = ["propertyId", "fromDate", "toDate", "tourType"]
