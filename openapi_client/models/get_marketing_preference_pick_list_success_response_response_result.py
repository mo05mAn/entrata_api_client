...
class GetMarketingPreferencePickListSuccessResponseResponseResult(BaseModel):
    """
    GetMarketingPreferencePickListSuccessResponseResponseResult
    """ # noqa: E501
    preferences: List[GetMarketingPreferencePickListSuccessResponseResponseResultPreferencesInner]
    availability_alert_frequencies: List[GetMarketingPreferencePickListSuccessResponseResponseResultAvailabilityAlertFrequenciesInner] = Field(alias="availabilityAlertFrequencies")
    __properties: ClassVar[List[str]] = ["preferences", "availabilityAlertFrequencies"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in availability_alert_frequencies (list)
        _items = []
        if self.availability_alert_frequencies:
            for _item in self.availability_alert_frequencies:
                if _item:
                    _items.append(_item.to_dict())
            _dict['availabilityAlertFrequencies'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "preferences": [GetMarketingPreferencePickListSuccessResponseResponseResultPreferencesInner.from_dict(_item) for _item in obj["preferences"]] if obj.get("preferences") is not None else None,
            "availabilityAlertFrequencies": [GetMarketingPreferencePickListSuccessResponseResponseResultAvailabilityAlertFrequenciesInner.from_dict(_item) for _item in obj["availabilityAlertFrequencies"]] if obj.get("availabilityAlertFrequencies") is not None else None
        })
        return _obj
