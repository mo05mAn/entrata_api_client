...
class GetSpecialsR4Method(BaseModel):
    """
    GetSpecialsR4Method
    """ # noqa: E501
    name: StrictStr
    version: StrictStr
    params: GetSpecialsR4MethodParams
    __properties: ClassVar[List[str]] = ["name", "version", "params"]

    @field_validator('name')
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['getSpecials']):
            raise ValueError("must be one of enum values ('getSpecials')")
        return value

    @field_validator('version')
    def version_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['r4']):
            raise ValueError("must be one of enum values ('r4')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "params": GetSpecialsR4MethodParams.from_dict(obj["params"]) if obj.get("params") is not None else None
        })
        return _obj
