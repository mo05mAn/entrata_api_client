...
class GetLeasePickListSuccessResponseResultLeaseFileTypes(BaseModel):
    """
    GetLeasePickListSuccessResponseResultLeaseFileTypes
    """ # noqa: E501
    lease_file_type: List[GetLeasePickListSuccessResponseResultLeaseFileTypesLeaseFileTypeInner] = Field(alias="leaseFileType")
    __properties: ClassVar[List[str]] = ["leaseFileType"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in lease_file_type (list)
        _items = []
        if self.lease_file_type:
            for _item in self.lease_file_type:
                if _item:
                    _items.append(_item.to_dict())
            _dict['leaseFileType'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "leaseFileType": [GetLeasePickListSuccessResponseResultLeaseFileTypesLeaseFileTypeInner.from_dict(_item) for _item in obj["leaseFileType"]] if obj.get("leaseFileType") is not None else None
        })
        return _obj
