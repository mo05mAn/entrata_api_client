...
class GetPropertyFeesMethod(BaseModel):
    """
    GetPropertyFeesMethod
    """ # noqa: E501
    name: StrictStr
    params: Optional[GetPropertyFeesMethodParams] = None
    __properties: ClassVar[List[str]] = ["name", "params"]

    @field_validator('name')
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['getPropertyFees']):
            raise ValueError("must be one of enum values ('getPropertyFees')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "name": obj.get("name"),
            "params": GetPropertyFeesMethodParams.from_dict(obj["params"]) if obj.get("params") is not None else None
        })
        return _obj
