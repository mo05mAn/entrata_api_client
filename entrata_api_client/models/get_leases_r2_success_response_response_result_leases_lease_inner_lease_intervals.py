...
class GetLeasesR2SuccessResponseResponseResultLeasesLeaseInnerLeaseIntervals(BaseModel):
    """
    List of lease intervals
    """ # noqa: E501
    lease_interval: List[GetLeasesR2SuccessResponseResponseResultLeasesLeaseInnerLeaseIntervalsLeaseIntervalInner] = Field(description="List of lease intervals", alias="leaseInterval")
    __properties: ClassVar[List[str]] = ["leaseInterval"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in lease_interval (list)
        _items = []
        if self.lease_interval:
            for _item in self.lease_interval:
                if _item:
                    _items.append(_item.to_dict())
            _dict['leaseInterval'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "leaseInterval": [GetLeasesR2SuccessResponseResponseResultLeasesLeaseInnerLeaseIntervalsLeaseIntervalInner.from_dict(_item) for _item in obj["leaseInterval"]] if obj.get("leaseInterval") is not None else None
        })
        return _obj
