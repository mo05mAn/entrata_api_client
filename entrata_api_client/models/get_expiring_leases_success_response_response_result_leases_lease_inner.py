...
class GetExpiringLeasesSuccessResponseResponseResultLeasesLeaseInner(BaseModel):
    """
    GetExpiringLeasesSuccessResponseResponseResultLeasesLeaseInner
    """ # noqa: E501
    attributes: GetExpiringLeasesSuccessResponseResponseResultLeasesLeaseInnerAttributes = Field(alias="@attributes")
    resident: GetExpiringLeasesSuccessResponseResponseResultLeasesLeaseInnerResident = Field(alias="Resident")
    unit: Optional[StrictStr] = Field(default=None, description="Unit number associated with the lease.", alias="Unit")
    lease_status: StrictStr = Field(description="Current status of the lease (e.g., Current, Notice).", alias="LeaseStatus")
    lease_starts: StrictStr = Field(description="Start date of the lease.", alias="LeaseStarts")
    lease_expires: StrictStr = Field(description="Expiration date of the lease.", alias="LeaseExpires")
    notice_date: Optional[StrictStr] = Field(default=None, description="Date the notice was given (if applicable).", alias="NoticeDate")
    __properties: ClassVar[List[str]] = ["@attributes", "Resident", "Unit", "LeaseStatus", "LeaseStarts", "LeaseExpires", "NoticeDate"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of resident
        if self.resident:
            _dict['Resident'] = self.resident.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "@attributes": GetExpiringLeasesSuccessResponseResponseResultLeasesLeaseInnerAttributes.from_dict(obj["@attributes"]) if obj.get("@attributes") is not None else None,
            "Resident": GetExpiringLeasesSuccessResponseResponseResultLeasesLeaseInnerResident.from_dict(obj["Resident"]) if obj.get("Resident") is not None else None,
            "Unit": obj.get("Unit"),
            "LeaseStatus": obj.get("LeaseStatus"),
            "LeaseStarts": obj.get("LeaseStarts"),
            "LeaseExpires": obj.get("LeaseExpires"),
