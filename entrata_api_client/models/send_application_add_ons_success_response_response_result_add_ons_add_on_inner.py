...
class SendApplicationAddOnsSuccessResponseResponseResultAddOnsAddOnInner(BaseModel):
    """
    SendApplicationAddOnsSuccessResponseResponseResultAddOnsAddOnInner
    """ # noqa: E501
    status: StrictStr = Field(description="Status of the add-on.")
    message: StrictStr = Field(description="Message related to the add-on.")
    id: StrictInt = Field(description="ID of the add-on.")
    add_on_id: StrictStr = Field(description="Unique identifier for the add-on.", alias="addOnId")
    node: StrictInt = Field(description="Node number associated with the add-on.")
    __properties: ClassVar[List[str]] = ["status", "message", "id", "addOnId", "node"]
