...
class GetExpiringLeasesMethodParams(BaseModel):
    """
    ...
    from_date: Optional[date] = Field(default=None, description="This is an optional field. This field accepts single value. Date from which leases are required", alias="fromDate")
    to_date: date = Field(description="This is a required field. This field accepts single value. Date upto which leases are required", alias="toDate")
    is_month_to_month: Optional[StrictInt] = Field(default=None, description="This is an optional field. This field accepts single value. Is Month To Month", alias="isMonthToMonth")
    is_wait_list: Optional[StrictInt] = Field(default=None, description="This is an optional field. This field accepts single value. If this parameter is enabled then we will show leases with lease type status notice and the property setting HIDE_NOTICE_RESIDENTS_FROM_RENE WALS should be disabled for this, else we will return error.", alias="isWaitList")
    __properties: ClassVar[List[str]] = ["propertyId", "fromDate", "toDate", "isMonthToMonth", "isWaitList"]

    @field_validator('is_month_to_month')
    def is_month_to_month_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    @field_validator('is_wait_list')
    def is_wait_list_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
