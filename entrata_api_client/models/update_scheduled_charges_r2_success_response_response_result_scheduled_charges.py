...
class UpdateScheduledChargesR2SuccessResponseResponseResultScheduledCharges(BaseModel):
    """
    UpdateScheduledChargesR2SuccessResponseResponseResultScheduledCharges
    """ # noqa: E501
    scheduled_charge: Dict[str, UpdateScheduledChargesR2SuccessResponseResponseResultScheduledChargesScheduledChargeValue] = Field(alias="scheduledCharge")
    __properties: ClassVar[List[str]] = ["scheduledCharge"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each value in scheduled_charge (dict)
        _field_dict = {}
        if self.scheduled_charge:
            for _key in self.scheduled_charge:
                if self.scheduled_charge[_key]:
                    _field_dict[_key] = self.scheduled_charge[_key].to_dict()
            _dict['scheduledCharge'] = _field_dict
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "scheduledCharge": dict(
                (_k, UpdateScheduledChargesR2SuccessResponseResponseResultScheduledChargesScheduledChargeValue.from_dict(_v))
                for _k, _v in obj["scheduledCharge"].items()
            )
            if obj.get("scheduledCharge") is not None
            else None
        })
        return _obj
