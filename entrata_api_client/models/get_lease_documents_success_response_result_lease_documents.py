...
class GetLeaseDocumentsSuccessResponseResultLeaseDocuments(BaseModel):
    """
    GetLeaseDocumentsSuccessResponseResultLeaseDocuments
    """ # noqa: E501
    lease_document: Dict[str, GetLeaseDocumentsSuccessResponseResultLeaseDocumentsLeaseDocumentValue] = Field(alias="LeaseDocument")
    __properties: ClassVar[List[str]] = ["LeaseDocument"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each value in lease_document (dict)
        _field_dict = {}
        if self.lease_document:
            for _key in self.lease_document:
                if self.lease_document[_key]:
                    _field_dict[_key] = self.lease_document[_key].to_dict()
            _dict['LeaseDocument'] = _field_dict
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "LeaseDocument": dict(
                (_k, GetLeaseDocumentsSuccessResponseResultLeaseDocumentsLeaseDocumentValue.from_dict(_v))
                for _k, _v in obj["LeaseDocument"].items()
            )
            if obj.get("LeaseDocument") is not None
            else None
        })
        return _obj
