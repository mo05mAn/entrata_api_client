...
class GetLeaseActivitiesSuccessResponseResponseResultEventsInner(BaseModel):
    """
    GetLeaseActivitiesSuccessResponseResponseResultEventsInner
    """ # noqa: E501
    event_id: StrictInt = Field(description="Unique identifier for the event", alias="eventId")
    type_id: StrictInt = Field(description="Type identifier for the event", alias="typeId")
    lease_id: StrictInt = Field(description="ID of the associated lease", alias="leaseId")
    lease_interval_id: StrictInt = Field(description="ID of the lease interval", alias="leaseIntervalId")
    type: StrictStr = Field(description="Type of the event")
    subtype_id: Optional[StrictInt] = Field(default=None, description="Subtype identifier for the event", alias="subtypeId")
    get_data_reference_id: StrictInt = Field(description="Reference ID for getting data", alias="getDataReferenceId")
    var_date: StrictStr = Field(description="Date of the event", alias="date")
    comments: Optional[StrictStr] = Field(default=None, description="Comments about the event", alias="Comments")
    event_reasons: Optional[StrictStr] = Field(default=None, description="Reasons for the event", alias="eventReasons")
    call_data: Optional[GetLeaseActivitiesSuccessResponseResponseResultEventsInnerCallData] = Field(default=None, alias="callData")
    __properties: ClassVar[List[str]] = ["eventId", "typeId", "leaseId", "leaseIntervalId", "type", "subtypeId", "getDataReferenceId", "date", "Comments", "eventReasons", "callData"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "getDataReferenceId": obj.get("getDataReferenceId"),
            "date": obj.get("date"),
            "Comments": obj.get("Comments"),
            "eventReasons": obj.get("eventReasons"),
            "callData": GetLeaseActivitiesSuccessResponseResponseResultEventsInnerCallData.from_dict(obj["callData"]) if obj.get("callData") is not None else None
        })
        return _obj
