...
class SendLeaseActivitiesSuccessResponseResultEventsEvent(BaseModel):
    """
    SendLeaseActivitiesSuccessResponseResultEventsEvent
    """ # noqa: E501
    node: StrictStr = Field(description="Node identifier for the event")
    lease_id: StrictStr = Field(description="Unique identifier for the lease", alias="leaseId")
    status: StrictStr = Field(description="Status of the event insertion")
    message: StrictStr = Field(description="Message detailing the event insertion result")
    __properties: ClassVar[List[str]] = ["node", "leaseId", "status", "message"]
